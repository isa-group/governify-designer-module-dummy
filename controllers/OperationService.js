...
exports.apiV2ModelsModelIdOperationsGET = function (args, res, next) {
    ...
    if (args.modelId) {
        ...
        if (manifest) {
            ...
            res.sendStatus(500);
            res.end('Something fails. It has not been impossible to load manifest.yaml');
        }
...
exports.apiV2ModelsModelIdOperationsOperationIdPOST = function (args, res, next) {
    ...
    if (!args.modelId || !args.operationId || !args.data) {
        ...
        if (model) {
            ...
            if (operation) {
                var query = {
                    details: args.details.originalValue,
                    format: args.format.originalValue
                };
                execOperation(operation.id, res, args.data.value, query);
            } else {
                res.status(404);
...
function execOperation(operationId, res, data, query) {

    try {
        operations[operationId](res, data, query);
    } catch (e) {
        res.sendStatus(500);
        res.end(e.toString());
    }
